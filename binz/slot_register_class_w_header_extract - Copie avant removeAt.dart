...
class WorkingSlot {
  ...
  WorkingSlot( List<String> workingSlotAsStringList ){
    ...
    total_decimal = double.parse(workingSlotAsStringList[1] ); // ancien workingSlotAsStringList[3]
    total_decimal = toNaiveDouble2digit(total_decimal); // separated line for digit round

    var taskLetter = workingSlotAsStringList[2]; //  ancien  workingSlotAsStringList[4];
    if ( taskLetter == 'trn') { 
      isTrainee = true ;
    }
    if ( taskLetter == 'Sim') { 
    ...
    notes_de_tache = workingSlotAsStringList[3];  //  ancien workingSlotAsStringList[9]
    notes_journee = workingSlotAsStringList[4]; // ancien workingSlotAsStringList[11]
...
class DayByDayRegister {
  ...
  DayByDayRegister (String fileName){ 
    ...
    //print('slotBySlotRegister.length234: ${slotBySlotRegister.length}');////////////////////////////////////////////////
  ...
  List slotBySlotRegisterFrom(List monthlyReportWithHeaderRaw){
    ...
    // 1) I should have done a list of Map <header, value> for each lines
    // 2) Here producing filtered working slot would have been more direct
    ...
    //var headerLine = removeAt(monthlyReportWithHeaderRaw);//
    ...
    for (var rawWorkingSlot in monthlyReportWithHeaderRaw){
      ...
      if (header) {//si ligne header      
        header = false; 
        var headerLine = rawWorkingSlot; // inutile
        ...
        for (var title in headerLine) {
          ...
          //print('slotBySlotRegister: ${slotBySlotRegister.length}');//////////////////////////////////////
      }
    }
