...
class WorkingSlot {
  ...
  WorkingSlot( List<String> workingSlotAsStringList ){
    //il faut creer une methode isValid
    date = workingSlotAsStringList[0]; // si vide alors WS INVALIDE
    if (date == '') { isValid = false; } // normalement ça bogue

    total_decimal = double.parse(workingSlotAsStringList[1] ); // si vide alors WS INVALIDE
    total_decimal = toNaiveDouble2digit(total_decimal); // separated line for digit round
    if (total_decimal == 0) { isValid = false; }

    var taskLetter = workingSlotAsStringList[2]; // si vide alors WS INVALIDE
    
    if (taskLetter.length == 3) {
      print('taskLetter.length == 3');
      switch (taskLetter[0]) {
        case 't':{
